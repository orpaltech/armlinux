...
if [ "${BOOTLOADER}" = uboot ] ; then
	...
	if [ -d "${UBOOT_SOURCE_DIR}" ] && [ -d "${UBOOT_SOURCE_DIR}/.git" ] ; then
		...
		sudo chown -R ${CURRENT_USER}:${CURRENT_USER} $UBOOT_SOURCE_DIR
                ...
                # update sources
		git -C ${UBOOT_SOURCE_DIR} fetch origin --depth=1 --tags
		[ $? -eq 0 ] || exit $?;
...
update_kernel()
{
	...
	if [ -d "${KERNEL_SOURCE_DIR}" ] && [ -d "${KERNEL_SOURCE_DIR}/.git" ] ; then
		...
		# update sources
		git -C ${KERNEL_SOURCE_DIR} fetch origin --depth=1 --tags
		[ $? -eq 0 ] || exit $?;
		...
		if [ -n "${KERNEL_REPO_BRANCH}" ] ; then
			...
			git clone ${KERNEL_REPO_URL} --depth=1 --tags ${KERNEL_SOURCE_DIR}
		fi
		[ $? -eq 0 ] || exit $?;
	fi
	...
	if [ -n "${fw_url}" ] ; then
		...
		if [ -d "${fw_src_dir}" ] && [ -d "${fw_src_dir}/.git" ] ; then
                        ...
                        sudo chown -R ${CURRENT_USER}:${CURRENT_USER} ${fw_src_dir}
                         ...
                         # update sources
                        git -C ${fw_src_dir} fetch origin --tags
                        [ $? -eq 0 ] || exit $?;
			...
			echo "Pulling updates out of branch: ${fw_branch}"
			...
			git clone ${fw_url} -b ${fw_branch} --tags ${fw_src_dir}
			[ $? -eq 0 ] || exit $?;
		fi

		if [ -n "${fw_tag}" ] ; then
			display_alert "Checking out firmware tag" "${fw_tag}" "info"
			git -C ${fw_src_dir} checkout tags/${fw_tag}
			[ $? -eq 0 ] || exit $?;
		fi
