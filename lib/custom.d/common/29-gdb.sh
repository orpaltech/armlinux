...
GDB_DEB_PKG_VER="${GDB_VERSION}-${SOC_FAMILY}-${VERSION}"
GDB_DEB_PKG="gdb-${DEBIAN_RELEASE_ARCH}-${GDB_DEB_PKG_VER}"
...
gdb_deb_pkg()
{
	...
	EOF
        ...
        dpkg-deb -z0 -b $GDB_DEB_DIR	"${BASEDIR}/debs/${GDB_DEB_PKG}.deb" 2> /dev/null
        [ $? -eq 0 ] || exit $?;
