...
pkg_config_libdir = '${SYSROOT_DIR}/usr/lib/${LINUX_PLATFORM}/pkgconfig:/usr/lib/pkgconfig:/usr/share/pkgconfig'
...
mesa_make()
{
	...
	# NOTE: only need vc4 and v3d, so remove other libraries
	mv ./dist/usr/lib/dri/vc4_dri.so ./dist/usr/lib/dri/vc4_dri.so.tmp
	mv ./dist/usr/lib/dri/v3d_dri.so ./dist/usr/lib/dri/v3d_dri.so.tmp
	rm -f ./dist/usr/lib/dri/*_dri.so
	mv ./dist/usr/lib/dri/vc4_dri.so.tmp ./dist/usr/lib/dri/vc4_dri.so
	mv ./dist/usr/lib/dri/v3d_dri.so.tmp ./dist/usr/lib/dri/v3d_dri.so
...
Description: This package provides Mesa 3D libraries for Broadcom BCM283x-family SoCs
EOF
...
if [ "${ENABLE_MESA}" = yes ] ; then
	...
	echo "Skip building MESA (vc4 backend) library."
fi
