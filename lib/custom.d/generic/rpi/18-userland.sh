...
USERLAND_FORCE_UPDATE=${USERLAND_FORCE_UPDATE:="no"}

userland_update()
{
	if [ "${USERLAND_FORCE_UPDATE}" = yes ] ; then
		echo "Force Raspberry-Pi userland update"
		rm -rf $USERLAND_DIR
	fi
	...
	if [ -d $USERLAND_DIR/.git ] ; then
		...
		if [ "${OLD_URL}" != "${USERLAND_URL}" ] ; then
			echo "Raspberry-Pi userland repository has changed, clean up working dir ?"
			pause
			rm -rf $USERLAND_DIR
		fi
	fi
	...
	USERLAND_DEB_PKG_VER="${USERLAND_DEB_VER}_${PROD_VERSION}-${CONFIG}_${DEBIAN_RELEASE_ARCH}"
...
userland_deb_pkg()
{
	echo "Create Raspberry-Pi userland deb..."
...
userland_deploy()
{
	echo "Deploying Raspberry-Pi userland..."
...
if [ "${SOC_ARCH}" = "arm" ] && [ "${ENABLE_RPI_USERLAND}" = "yes" ]; then
	...
	echo "Skip Raspberry-Pi userland build for ${SOC_ARCH}"
fi
