...
if [ "${DEBIAN_NONFREE}" = yes ] ; then
  sed -i "s/ main/ main contrib non-free non-free-firmware/"	${ETC_DIR}/apt/sources.list
fi

# Allow the installation of experimental Debian packages
if [ "${DEBIAN_EXPERIMENTAL}" = yes ] ; then
  printf "\ndeb http://${APT_SERVER}/debian sid main" >> ${ETC_DIR}/apt/sources.list
fi
