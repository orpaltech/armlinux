...
${UBOOT_SOURCE_DIR}/tools/mkimage -A "${KERNEL_ARCH}" -O linux -T ramdisk -C none -n uInitrd \
	-d "${INITRD_IMAGE_FILE}" "${BOOT_DIR}/uInitrd"
...
${UBOOT_SOURCE_DIR}/tools/mkimage -A "${KERNEL_ARCH}" -O linux -T script -C none -a 0x00000000 -e 0x00000000 \
	-d "${FELBOOT_BOOTSCR_SRC}" "${BOOT_DIR}/boot-fel.scr"
